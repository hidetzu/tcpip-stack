...
|---|---|---|
| Interpreting anything above the internet header | ⚠ **no** | ⚠ Not yet written, and not a decision. The Parse layer reads the ethernet header, an ARP packet and an IPv4 header, and stops there |
...
| ICMP, and anything an accepted datagram should cause | ⚠ **no** | ⚠ Not yet written. Not a decision — the milestone is to answer a ping, and ⚠ **reading a header is not answering one**: nothing calls `ipv4_parse_header` outside its checks yet, and `ping` still reports 100% packet loss, measured 2026-08-27 |
...
| Cost of `make check-static` | 4083 ms from a clean tree; 170 ms and 171 ms with the build already done | 2026-08-28 | 3 runs, all three values listed. ⚠ **Eleven cases, six of them C check binaries built with the sanitizers** — ⚠ re-measured because that denominator moved (hidetzu/tcpip-stack#33); ⚠ the row it replaced said nine cases and four binaries, and ⚠ **it was already one case behind when it was read** |
