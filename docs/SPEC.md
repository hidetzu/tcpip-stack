...
| Routing, a routing table, a neighbour cache | yes | ⚠ A reply goes back to the ethernet and IPv4 addresses the frame that asked came from, ⚠ **read out of that frame and never out of a table this stack does not keep**. ⚠ So it answers whoever is on the other end of the device and nothing else |
| ⚠ Anything TCP or UDP | ⚠ **no** | ⚠ Not yet written, and not a decision. ⚠ A datagram carrying either comes back as a protocol we do not act on, counted apart from every other reason |
...
| Cost of `make check-static` | 6738 ms from a clean tree; 235 ms and 240 ms with the build already done | 2026-08-28 | 3 runs, all three values listed. ⚠ **Thirteen cases, seven of them C check binaries built with the sanitizers** — ⚠ re-measured because that denominator moved again (hidetzu/tcpip-stack#35) |
| Cost of `make check-real` | 1331 / 666 / 670 ms | 2026-08-28 | 3 runs, all three values listed. ⚠ The first is from a tree whose build was not up to date and the other two are not; ⚠ all three are listed rather than the two that agree (`CLAUDE.md` §6). Eight cases, each with its own namespace |
| Cost of `make check-foreign` | 5630 / 5634 / 5630 ms | 2026-08-28 | 3 runs, all three values listed. ⚠ **Four cases** now, each with its own namespace and its own pings — ⚠ re-measured because that denominator moved (hidetzu/tcpip-stack#35) |
