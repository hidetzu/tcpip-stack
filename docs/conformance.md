...
| ⚠ **What is left of the retransmission schedule** | `MUST-20` (b) | ⚠ **`MUST-18` left at hidetzu/tcpip-stack#130, `MUST-23` at #131, `MUST-19` at #132.** ⚠ **One clause remains**: negative advice to the IP layer at R1. ⚠ **R1 exists, is crossed, counted and said**; ⚠ **there is no IP layer here to advise**, nothing routes and there is no gateway to diagnose |
...
| ID | Requirement, quoted | Verdict |
|---|---|---|
| `MUST-4` | "Support the mandatory option set" | ⚠ **met** since hidetzu/tcpip-stack#126 — ⚠ **the set is exactly `{MSS}`**, and `MUST-14` is met. ⚠ **This row follows `MUST-14` rather than leading it**, as it did when both were not met |
...
| `MUST-14` | "TCP endpoints MUST implement both sending and receiving the MSS Option" | ⚠ **met** since hidetzu/tcpip-stack#126, ⚠ **and only because the consumer now exists.** ⚠ The option is **sent** on the `SYN,ACK` from the device's MTU and one that **arrives is read** (hidetzu/tcpip-stack#123) — ⚠ **that alone was recorded as NOT met**, on hidetzu/tcpip-stack#123's Owner Decision: ⚠ **「受信値が実際の SendMSS / effective send MSS に使われ、送信 segment の大きさを制約する consumer まで接続された時点で再判定してください」.** ⚠ **That consumer is `MUST-16` and it is met now**: the value they send bounds the segments we send, ⚠ **measured on the wire against the Linux kernel.** ⚠ **Re-judged on the owner's own standard, not on it having become convenient** |
...
| `MUST-15` (IPv4) | "If an MSS Option is not received at connection setup, TCP implementations MUST assume a default send MSS of 536 (576 - 40) for IPv4" | ⚠ **met** since hidetzu/tcpip-stack#126. ⚠ **A `SYN` with no MSS Option leaves 536 and a flag saying it was assumed** — ⚠ **absent and "they said 536" are kept apart** — ⚠ **and the assumed value now bounds what we send**, through `MUST-16`. ⚠ **It was met in the assuming and not in the using until the consumer existed** |
| `MUST-15` (IPv6) | "or 1220 (1280 - 60) for IPv6" | ⚠ **does not arise** — no IPv6 |
| `MUST-16` | "The maximum size of a segment that a TCP endpoint really sends, the 'effective send MSS', MUST be the smaller (MUST-16) of the send MSS ... and the largest transmission size permitted by ..." | ⚠ **met** since hidetzu/tcpip-stack#126. ⚠ **The smaller of the two, per connection** — what they told us (or `MUST-15`'s 536) and what one of our frames carries. ⚠ **Measured against the Linux kernel**: 3000 octets handed to this stack at MTU 1400 ⚠ **reached the kernel's own `read()` in three segments of 1360, 1360 and 280** — ⚠ **and they were the right 3000 octets**, checked against the pattern. ⚠ `tests/interop.sh` `data_larger_than_the_mss_arrives_in_segments_it_bounds`, `tests/static.sh` `handshake` → `the_effective_send_mss_is_the_smaller_of_the_two`. ⚠ **The criterion was set before any of it was built** (hidetzu/tcpip-stack#125): ⚠ **「5 bytes を送れることではなく…」** — ⚠ **and a check that only added the octets up would pass for a stack that sent all 3000 in one segment** |
...
| ID | Requirement, quoted | Verdict |
|---|---|---|
| `MUST-19` | "A TCP endpoint MUST implement the basic congestion control algorithms slow start, congestion avoidance, and exponential backoff of RTO to avoid creating congestion collapse conditions" — ⚠ **RFC 5681 admitted for the first two, RFC 6298 for the third** (ADR 0024 as amended) | ⚠ **met** since hidetzu/tcpip-stack#132. ⚠ **All three, and each asserted apart**: ⚠ **slow start** grows `cwnd` by at most `SMSS` per acknowledgment of new data (RFC 5681 §3.1's `MUST`, by its RECOMMENDed equation 2); ⚠ **congestion avoidance** grows it more slowly, by the equation 3 the document says a TCP `MAY` use, ⚠ **and a check tells the two regimes apart**; ⚠ **exponential backoff** since hidetzu/tcpip-stack#131. ⚠ **`IW` follows §3.1's table at all three boundaries and one octet either side**, ⚠ **`ssthresh` on a timeout is `max(FlightSize/2, 2*SMSS)` and a check asserts it is NOT `cwnd/2`** — the document calls that "an easy mistake to make" — and ⚠ **`cwnd` becomes one full-sized segment.** ⚠ **Nothing is sent past `SND.UNA + min(cwnd, rwnd)`**, asserted with each of the two smaller in turn. ⚠ **Measured on the wire**: with one segment dropped, all 30000 octets still arrived and the window was cut and reopened. ⚠ **Fast retransmit and fast recovery are NOT this requirement** — `MUST-19` names three algorithms and they are not among them (ADR 0024's scope table) |
| `MUST-18` | "The RTO MUST be computed according to the algorithm in [10], including Karn's algorithm for taking RTT samples" — ⚠ **[10] is RFC 6298, admitted to the baseline 2026-08-29** (ADR 0024 as amended) | ⚠ **met** since hidetzu/tcpip-stack#130. ⚠ **§2.1's initial value, §2.2's first sample, §2.3's later ones IN THE ORDER IT MAKES A `MUST`, §2.4's floor, §2.5's ceiling, §4's `G` — measured, not assumed** — and ⚠ **§3's Karn's algorithm: no sample is taken from anything sent more than once, and refused samples are counted apart.** ⚠ **The fixed interval is gone and a check stops it coming back** (`tests/static.sh` `the_borrowed_interval_is_gone`). ⚠ `tests/static.sh` `handshake` → `the_timeout_is_computed_the_way_the_document_says` and `no_round_trip_is_measured_from_a_retransmission`. ⚠ **`MUST-19`'s exponential backoff is NOT this requirement and is not met** (hidetzu/tcpip-stack#131) |
...
| ID | Requirement, quoted | Verdict |
|---|---|---|
| `MUST-20` (R1) | §3.8.3 (b): "When the number of transmissions of the same segment reaches or exceeds threshold R1, pass negative advice (see Section 3.3.1.4 of [19]) to the IP layer, to trigger dead-gateway diagnosis" | ⚠ **not met, ⚠ and it moved.** ⚠ **R1 exists** since hidetzu/tcpip-stack#131 — `SHLD-10`'s own three retransmissions — ⚠ **the threshold is crossed, counted and said, once.** ⚠ **What is not met is the clause itself**: ⚠ **there is no IP layer here to advise**, nothing routes and there is no gateway to diagnose (`docs/SPEC.md` §2). ⚠ **A line on a terminal is not negative advice to a routing layer**, and calling it one would be the shape `CLAUDE.md` §1 forbids |
| `MUST-20` (R2) | §3.8.3 (c): "When the number of transmissions of the same segment reaches a threshold R2 greater than R1, close the connection" | ⚠ **met** since hidetzu/tcpip-stack#131. ⚠ **R1 exists now**, so there is an R2 greater than it: ⚠ **R1 is `SHLD-10`'s three retransmissions and R2 is a time** — `SHLD-11`'s 100 seconds for data, `MUST-23`'s 180 for a `SYN`. ⚠ **§3.8.3 (a) offers both units** — "R1 and R2 might be measured in time units or as a count of retransmissions" — ⚠ **and each is expressed in the unit the sentence that gives its value uses**, so no conversion of ours stands between them and the document. ⚠ hidetzu/tcpip-stack#114 corrected a verdict that called the time form wrong |
| `MUST-21` | "An application MUST be able to set the value for R2 for a particular connection" | ⚠ **does not arise** — ⚠ **there is no application** (ADR 0022). ⚠ **R2 exists now** and nothing can set it |
| `SHLD-9` | "TCP implementations SHOULD inform the application of the delivery problem ... when R1 is reached and before R2" | ⚠ **does not arise** — ⚠ **there is no application** (ADR 0022). ⚠ **R1 IS reached and said** since hidetzu/tcpip-stack#131; ⚠ **a human reading a terminal is not the application this asks for** |
| `SHLD-10` | "The value of R1 SHOULD correspond to at least 3 retransmissions, at the current RTO" | ⚠ **met** since hidetzu/tcpip-stack#131 — `HANDSHAKE_R1_RETRANSMISSIONS` is **3**, ⚠ **the document's own number** |
| `SHLD-11` | "The value of R2 SHOULD correspond to at least 100 seconds" | ⚠ **met** since hidetzu/tcpip-stack#131 — 100000 ms for data, ⚠ **and 180000 for a `SYN`, which `MUST-23` puts a higher floor under** |
...
| `MUST-23` | "R2 for a SYN segment MUST be set large enough to provide retransmission of the segment for at least 3 minutes" | ⚠ **met** since hidetzu/tcpip-stack#131. ⚠ `HANDSHAKE_R2_FOR_A_SYN_MILLISECONDS` is **180000**. ⚠ **Owner Decision 2026-08-29**: 「SYN R2 を 180 秒以上とします。ただし通常の verification で実時間3分を毎回待つことは要求しません。」 ⚠ **No check waits three minutes and none needs a seam to avoid it**: ADR 0018 hands the State layer a moment rather than letting it read one, ⚠ **so `at(180000)` and one millisecond either side judge the boundary in microseconds** — ⚠ **the production signature, not a test hook.** ⚠ **The device-tier cases that waited the old three seconds out are repointed at the retransmissions**, which is what a device is for. ⚠ `tests/static.sh` `handshake` → `a_connection_nobody_confirms_is_given_up_on`, asserted against the constant **and** against 180000 |
...
| ID | Requirement, quoted | Verdict |
|---|---|---|
| `MUST-54` | "TCP implementations MUST act on an ICMP error message passed up from the IP layer, directing it to the connection that created the error. The necessary demultiplexing information can be found in the IP header contained within the ICMP message" | ⚠ **met** since hidetzu/tcpip-stack#138. ⚠ **The error is matched by what it quotes back** — the addresses from the internet header and ⚠ **the ports from the first 64 bits of the datagram's data**, which RFC 792 says are there for exactly this: "This data is used by the host to match the message to the appropriate process." ⚠ **Measured against the Linux kernel**: `nft ... reject with icmp type` makes it send a real one, ⚠ **and the connection it names reacts.** ⚠ `tests/interop.sh` `an_icmp_error_the_kernel_sent_reaches_the_connection`, `tests/static.sh` `handshake` → `each_class_of_icmp_error_does_its_own_thing`, `icmp_message` → `an_error_message_is_read_or_refused_for_its_own_reason` |
| `MUST-55` | "TCP implementations MUST silently discard any received ICMP Source Quench messages" | ⚠ **met, ⚠ and no longer by accident** (hidetzu/tcpip-stack#138). ⚠ **It was met because nothing acted on any ICMP error at all** — ⚠ **it would still have been met if the whole ICMP layer were deleted.** ⚠ **The message reaches the connection now and is dropped there, by a line that says why.** ⚠ "Silently" forbids a segment on the wire and ⚠ **not a line telling the human**, the reading `MUST-57` took at hidetzu/tcpip-stack#99 — ⚠ **and a check asserts no segment leaves** |
| `MUST-56` | "Since these Unreachable messages indicate soft error conditions, a TCP implementation MUST NOT abort the connection" | ⚠ **met, ⚠ and no longer vacuously** (hidetzu/tcpip-stack#138). ⚠ **It was met by never being in a position to abort.** ⚠ **A soft error reaches the connection now and leaves it alone**, and ⚠ **a check asserts the connection is still there and a hard one is not** — ⚠ so the two cannot be confused |
| `SHLD-25` | "and it SHOULD make the information available to the application" | ⚠ **does not arise** — ⚠ **there is no application** (ADR 0022). ⚠ **The information exists now** (hidetzu/tcpip-stack#138) and ⚠ **a human reading a terminal is not the application this asks for** |
| `SHLD-26` | "These are hard error conditions, so TCP implementations SHOULD abort the connection" | ⚠ **met** since hidetzu/tcpip-stack#138 — Destination Unreachable codes 2 to 4 end the connection. ⚠ **The document names the doubt in the same breath**: "[35] notes that some implementations do not abort connections when an ICMP hard error is received for a connection that is in any of the synchronized states." ⚠ **Taken anyway, and that is a decision** (ADR 0035) |
