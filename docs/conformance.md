...
| ⚠ **The retransmission schedule** | `MUST-19`, `MUST-20` (b) | ⚠ **`MUST-18` left at hidetzu/tcpip-stack#130 and `MUST-23` at #131.** ⚠ **What remains is two things and they are not one gap**: ⚠ `MUST-19`'s slow start and congestion avoidance (hidetzu/tcpip-stack#132) — ⚠ **its exponential backoff IS met** since #131 — and ⚠ **`MUST-20` (b)'s negative advice to IP, which has nothing here to advise** |
