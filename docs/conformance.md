...
| ⚠ **The retransmission schedule** | `MUST-19`, `MUST-20`, `MUST-23` | ⚠ **`MUST-18` left this group at hidetzu/tcpip-stack#130** — the RTO is computed from measured round trips now. ⚠ **What remains is one gap seen from three sides**: the timeout does not double on an expiry (RFC 6298 §5.5), ⚠ **there is no R1 at all** — so no "threshold R2 greater than R1" — and the give-up time is three seconds against `MUST-23`'s three minutes. ⚠ **Owner Decision 2026-08-29: `MUST-23` is 180 seconds, and no check waits three real minutes for it** — ⚠ the production time model judges the boundary with no waiting (ADR 0018) |
