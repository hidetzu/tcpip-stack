...
experiment in AI-assisted systems engineering. ⚠ **v0.1.0 deliberately has no application-facing TCP
API and no routing-layer consumer** (ADR 0037).
...
| ⚠ **`docs/SPEC.md` §2 kept saying a gap was open after it was closed — four times.** ⚠ hidetzu/tcpip-stack#126, #130, #131 and #132 each closed one, ⚠ **and each left §2's old sentence standing**: "nothing here measures a round trip", "it retransmits at the same interval for ever", "none exists", "three seconds". ⚠ **`CLAUDE.md` §5 lists the set to update — implementation → test → comment → README → `docs/SPEC.md`** — ⚠ **and the last one was dropped every time.** ⚠ **§2 is the file that owns what may NOT be claimed**, so ⚠ **a stale row there makes this stack look like it does LESS than it does** — ⚠ the opposite direction from over-claiming and ⚠ **still a false statement in the file that exists to hold them.** ⚠ **README was stale in the other direction at the same time**: "Nothing else is sent", after `--send` began sending. ⚠ **Found while deciding whether the tree was fit to call v0.1.0, not by any check** | ⚠ **Wall:** `tests/static.sh` `spec_does_not_deny_what_conformance_says_is_met` — ⚠ **a §2 row citing a requirement `docs/conformance.md` records as met must say it is met, and must not say it is not.** ⚠ **It found eight rows on its first run**, ⚠ **five of which were honest citations that did not say which** — ⚠ so the rule makes the document better rather than working around the check. ⚠ **It cannot read whether a sentence is true**, only whether the two files contradict each other about a requirement they both name — ⚠ **which is the half that can be mechanised, and it is the half that failed** |
